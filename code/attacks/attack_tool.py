	...
	def start_attack(self, device_num: int, attack_type: AttackType):
		if self.get_attack_list_entry(device_num, attack_type) is None:
			...
			if preventing_attack is None:
				# We flag the start of the attack before actually attempting to start it, since the SSH connections
				# involved at the start of an attack are also part of its power signature and should be labelled as
				# such.
