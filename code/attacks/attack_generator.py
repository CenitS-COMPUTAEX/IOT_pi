...
from defs.attack_event.attack_event_factory import AttackEventFactory
...
class AttackGenerator:
	...
	# If > 0, attacks will be generated as if this many minutes had already passed
			...
			while len(self.events) > 0 and not self._should_stop():
				...
				while True:
					if self._should_stop():
						break
					wait_time = event.get_time() - time.time()
					if wait_time <= 0:
						break
	...
	def create_event_list(self, seed: int = None) -> List[AttackEvent]:
		...
		start_time = time.time() - self.continue_minutes * 60
		end_time = start_time + self.duration * 60
		event_factory = AttackEventFactory(start_time)
			...
			while current_time < end_time:
				...
				for i in range(num_attacks):
					if len(possible_attacks) > 0:
						...
						attack_event = event_factory.start_event(current_time, device_num, attack)
						# Store in event list
						self.events.append(attack_event)
						active_attacks.append(attack_event)

				# The flood attack is the most resource-demanding attack, and starting it first might prevent the rest
				# of the attacks from successfully starting, so we move it to the last position if there's multiple.
				active_attacks_no_reorder = active_attacks.copy()
				self._move_attack_to_end_position(active_attacks, AttackType.FLOOD)

				# Stop each attack after a random amount of time
				# We need to use the list prior to the reorder operation, since it was introduced in a later version.
				# Otherwise, the same random seed would result in a different event list.
				num_attacks = len(active_attacks_no_reorder)
				...
				for duration in durations:
					...
					attack_to_end = active_attacks_no_reorder.pop(random.randrange(len(active_attacks_no_reorder)))
					self.events.append(
						event_factory.end_event(current_time + duration, attack_to_end.device_num, attack_to_end.attack))

				current_time += durations[-1]
				assert(current_time >= self.events[-1].get_time())
		# End all remaining attacks
		self.events.append(event_factory.end_all_event(end_time))

		# Sort the list by event time
		self.events.sort(key=cmp_to_key(lambda ev1, ev2: ev1.get_time() - ev2.get_time()))
		# Remove past events (can happen if self.continue_minutes is > 0)
		self._remove_past()
		return self.events
	...
	def _remove_past(self):
		"""
		Removes entries from the event list with a time previous to the current timestamp. The list is assumed to be
		sorted.
		"""
		i = 0
	...
	def _contains_attack_start(self, events: List[AttackEvent], attack_type: AttackType) -> bool:
		"""
		Returns true if the given event list contains an attack start event for the given attack type
		"""
		for event in events:
			if event.action == AttackAction.START and event.attack == attack_type:
				return True
		return False

	def _move_attack_to_end_position(self, current_events: List[AttackEvent], attack_to_move: AttackType):
		"""
		Checks the events that were just added to the event list. If multiple were just added and one of them is
		a start event for the specified attack, moves it to the end of the list, both on the provided list and on
		this class' event list.
		current_events: List containing the events that were just added to the list (one or multiple attack start
		events). The elements contained here are expected to be the same ones currently at the end of the event list.
		"""
		if len(current_events) > 1 and self._contains_attack_start(current_events, attack_to_move) \
		and current_events[-1].attack != attack_to_move:
			# Reordering required
			for i in range(len(current_events)):
				event = current_events[i]
				if event.attack == attack_to_move and event.action == AttackAction.START:
					attack = current_events.pop(i)
					current_events.append(attack)

					# Move it in the event list too
					index_event_list = len(self.events) - len(current_events) + i
					attack2 = self.events.pop(index_event_list)
					assert attack == attack2
					self.events.append(attack2)
					break


# if __name__ == "__main__":
# 	generator = AttackGenerator([1, 2, 3], 60 * 5, NormalParams(20, 10))
# 	generator.create_event_list()
