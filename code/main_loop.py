...
def main():
	args = sys.argv
	if os.name == "nt":
		# Required to catch KeyboardInterrupt if the program is run as a subprocess on Windows
		ctypes.windll.kernel32.SetConsoleCtrlHandler(None, 0)
	elif os.name == "posix":
		# Required to receive SIGINT when running as a background process on Unix
		...
		if "-a" in args:
			...
			if value is not None:
				...
				if params.mean <= 0 or params.std <= 0:
					print("Error: Attack duration params must be > 0")
					return 1
			...
			if value is not None:
				float_val = float(value)
				if float_val <= 0:
					print("Error: Minimum attack duration must be > 0")
					return 1
			...
			if value is not None:
				float_val = float(value)
				if float_val <= 0:
					print("Error: Minimum attack delay must be > 0")
					return 1
			...
			if value is not None:
				float_val = float(value)
				if float_val <= 0:
					print("Error: Short attack duration must be > 0")
					return 1
				generator.short_attack_scale = float_val
			value = pop_flag_param(args, "--continue")
			if value is not None:
				int_val = int(value)
				if int_val <= 0:
					print("Error: Continue minutes must be > 0")
					return 1
				generator.continue_minutes = int_val
...
def run(generator: "AttackGenerator | None", print_events: bool, exit_early: bool, buffer_size: int, log_attacks: bool,
	...
	if generator is None:
		...
		end_time = time.time() + (generator.duration - generator.continue_minutes) * 60
	...
	print("Flags: \n"
		...
		"parameters:\n"
			...
			"\t--attacks: List of comma-separated values indicating the attacks that can be started. Possible values:\n"
				...
				"\t  flood (5): TCP SYN flood attack\n"
				"\t  port-scan (6): Port scanning attack\n"
			...
			"\t--continue <minutes>: Pretend the program has already been running for <minutes> minutes. Affects the "
			"duration of the test and the generated attacks. Useful to resume a previously interrupted run.\n"
