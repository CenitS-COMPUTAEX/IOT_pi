...
- [IOT](https://github.com/CenitS-COMPUTAEX/IOT/tree/v2): Contains scripts used to train and test one or more Machine Learning models, as well as scripts used to test and run them. Models can be run offline (with an existing dataset) or online (reading real-time data from a buffer).
...
This repository contains the scripts that are run on the main and end devices in order to read power usage and launch attacks.
...
- [code](code): Scripts
	...
	- [defs](code/defs): General class definitions
...
- [attack_scripts](attack_scripts): Source code of the scripts used that implement other attacks
	- [crypt](attack_scripts/crypt): Source code of the encryption attack (C++)
	- [flood](attack_scripts/flood): Source code of the flood attack (C)
...
- Flood: Sends a large amount of SYN requests to the target device, overwhelming it.
- Port Scan: Performs connection attempts to the target device on each possible port. This attack doesn't have a large effect on the power usage of the target device, but it does generate a lot of network traffic.
...
- Encryption attack
	...
	- The executable of the encryption program must be able to run on the **end** device.
		- To ensure this, compile the encryption program on the end device using the source code, which can be found in the [attack_scripts/crypt](attack_scripts/crypt) folder.
...
- Flood attack
	- The **main** device must have the executable of the flood attack located at `Config > Attacks > Flood > FileToRun`.
	- The executable must be able to run on the **main** device.
	- The executable must have permission to create raw network sockets. On Unix systems, this can be achieved by running `sudo setcap cap_net_raw+ep path_to_flood_executable`.
- Port Scan attack
...
- When launching attacks while recording a dataset, if the network is experiencing congestion, it's possible that the attack script might not be able to connect to the remote device to start the attack. After a few failed attempts, the attack will be aborted. However, the dataset instances created while this happened will still be marked as an attack, even though the attack didn't actually happen. These incorrect instances should be manually edited or removed from the dataset by editing the resulting file.
  - If this situation happens, the following message will be logged: "If a dataset was being created, some incorrect instances have been introduced as a result of this, since the attack wasn't actually run.".
  - In order to remove those instances, create a backup of the dataset file, open it with a text editor and look for the last valid instance you want to keep. Each instance is composed of multiple lines, with the last line of each instance containing its timestamp. This timestamp can be helpful to determine which instances to remove. 
...
You must follow these steps to implement a new attack:
